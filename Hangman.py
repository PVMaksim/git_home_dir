...
def get_word():  # случайный выбор слова
    text = choice(word_rus).upper()
    return text


def change_text_char(text, index, char):
    change_text_char = text[:index] + char + text[index + 1:]
    return change_text_char


def find_letter(find_text):
    global guessed
    global guessed_letters
    global word_completion
    while guessed == 0:  # угадываем буквы
        letter = input('Напиши букву: ')
        if letter not in АБВ:  # проверка на принадлежность к русскому алфавиту
            print('Тебе нужно написать всево лишь букву. Попробуй еще раз.')
        else:
            if letter.upper() not in find_text:  # проверка угадал ли букву
                print('Ты не угадал букву. Попробуй еще раз.')
                if letter not in guessed_letters:
                    guessed_letters += [letter]
                print("Эти буквы ты уже называл *", *guessed_letters, '*')
            else:  # если угадал букву
                if letter.upper() in guessed_letters:  # проверка на повторный ввод буквы
                    print("Напиши другую букву. Эту ты уже угадал.")
                else:
                    if find_text.count(letter.upper()) == 1:  # если буква в слове одна
                        word_completion = change_text_char(
                            word_completion, find_text.index(letter.upper()), letter.upper())
                        if letter.upper not in guessed_letters:
                            guessed_letters += [letter.upper()]
                        print(word_completion)
                        guessed = 1
                    else:
                        for i in range(len(find_text)):  # если букв несколько одинаковых
                            if find_text[i] == letter.upper():
                                word_completion = change_text_char(
                                    word_completion, i, letter.upper())
                        print(word_completion)
                        guessed = 1
                return word_completion, guessed_letters


def find_word(find_text):
    global guessed
    global guessed_letters
    global word_completion
    ...
    global tries
    global word
    print('Ты угадал букву теперь попробуй угадать все слово: ')
    while guessed == 1:  # угадываем слова
        word = input()
        if not word.isalpha():  # проверка на то что нет символов и цифр
            print('Попробуй еще раз написать слово.')
            word = input()
        else:
            if word.upper() != find_text:  # проверка угадал ли слово
                print('Ты не угадал слово. Попробуй угадать еще букву.')
                tries -= 1
                print(display_hangman(tries))
                if word not in guessed_words:  # добавляем слово к списку написанных слов
                    guessed_words += [word]
                print("Эти слова ты уже называл *", *guessed_words, '*')
                guessed = 0
            else:  # если угадал слово
                print('Поздравляю ты угадол слово!')
                guessed = 0
            return word_completion, guessed_letters
...
def play():
    ...
    print(display_hangman(tries))
    print(word_completion, 'я загадал слово из', len(find_text), 'букв')
    print("Попробуй его угадоть. Ты называешь букву и если ты ее угадал, я покажу где она расположуна в слове. ")
    print("После каждой угаданной буквы ты должен попробывать угадать слово.")
    print("У тебя есть шесть попыток. Начнем!")
    while play_go == 'да':
        while word.upper() != find_text and tries != 0:
            find_letter(find_text)
            find_word(find_text)
        print('Ты не угадал слово за шесть попыток.')
        play_go = input('Сыграем гще? Напиши да или нет')
...
word_completion = '_' * len(find_text)
guessed = 0  # сигнальная метка
